...
export default function Topbar() {
  ...
  const showBadge = ['/record', '/drill-record', '/ladder-record', '/slide-record', '/processing', '/results'].some((p) =>
      ...
      <div className="hidden lg:flex items-center gap-2 shrink-0">
        {showBadge && (
          <div className="text-[11px] bg-accent-dim border border-accent-border text-accent px-3 py-1 rounded-full tracking-[0.02em] font-medium max-w-[140px] truncate">
            {selectedContext || (sessionType === 'slide' ? 'Slide Deck' : sessionType === 'ladder' ? 'Topic Ladder' : sessionType === 'drill' ? 'Question Drill' : 'Free Talk')}
          </div>
        )}
      </div>
    </nav>
