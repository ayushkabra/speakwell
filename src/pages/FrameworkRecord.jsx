...
import { getFrameworkById } from '../lib/frameworkGenerator';
...
export default function FrameworkRecord() {
  ...
  const framework = getFrameworkById(frameworkType);
  const steps = framework.steps;
  const currentStepObj = steps[currentFrameworkStep] || steps[0];
  const totalSteps = steps.length;
  const isLastStep = currentFrameworkStep === totalSteps - 1;
  ...
  const handlePreviousStep = () => {
    ...
    if (fullTranscript || elapsed > 0) {
      ...
      addFrameworkAnswer({
        ...
        stepNum: currentStepObj.stepNum,
  ...
  // Advance to Next Step or Complete Framework Session
  const handleNextOrFinish = async () => {
    ...
    if (isLastStep) {
      ...
      const combinedTranscript = allAnswers
        .map((a) => `[STEP ${a.stepNum}: ${a.title}]: ${a.transcript}`)
        .join('\n\n');
      ...
      if (combinedTranscript.length > 15) {
        polishResult = await polishTranscript(combinedTranscript, `${framework.title}: ${frameworkPrompt}`);
      }
      ...
      addSession({
        context: `${framework.title}: ${frameworkPrompt || framework.subtitle}`,
        ...
        polishedScript: polishResult.polished || combinedTranscript,
      ...
      <div className="grid grid-cols-[48%_52%] gap-8 items-start max-[900px]:grid-cols-1">
        ...
        <div className="flex flex-col gap-5 bg-surface border border-border-md rounded-2xl p-7 shadow-xl">
          ...
          <div className="flex items-center justify-between border-b border-border/60 pb-4">
            <div className="flex items-center gap-2">
              ...
              <span className="bg-accent/15 text-accent border border-accent/30 rounded-full px-3 py-1 text-[11px] font-semibold uppercase tracking-wider">
                Step {currentFrameworkStep + 1} of {totalSteps}
              </span>
            </div>
          ...
          <div className="text-[12px] text-text3 italic">
            Topic: <strong className="text-text">{frameworkPrompt || framework.title}</strong>
          </div>

          {/* Step Display Card */}
          <div className="p-6 bg-surface2/60 border border-border rounded-xl flex flex-col justify-center min-h-[160px]">
            <div className="text-[10px] tracking-[0.2em] uppercase text-accent mb-2 font-medium">
              {framework.title} · Step {currentStepObj.stepNum}
            </div>
            <h2 className="font-serif text-[24px] leading-[1.35] text-text font-normal mb-3">
              "{currentStepObj.title}"
            </h2>
            <div className="p-3.5 bg-surface border border-border/70 rounded-lg text-[13px] text-text2 leading-[1.6]">
              💡 {currentStepObj.guide}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-3 pt-2">
            <div className="flex items-center gap-2 flex-wrap">
              {!isRecording && !transcript ? (
                ...
                >
                  🎙 Speak Step {currentStepObj.stepNum}
                </button>
              ) : (
                <>
                  <button
            ...
            <div className="flex items-center justify-between gap-3 pt-2">
              ...
              >
                {isLastStep ? 'Finish Framework Rehearsal →' : 'Next Step →'}
              </button>
            </div>
          </div>
        </div>

        {/* RIGHT COLUMN: Live Spoken Teleprompter Box */}
          ...
          <div>
            <div className="flex items-center justify-between mb-4 border-b border-border/60 pb-3">
              <div className="text-[11px] tracking-[0.18em] uppercase text-text3 font-medium flex items-center gap-2">
                <span>Spoken Thought (Step {currentStepObj.stepNum})</span>
                {isRecording && (
            ...
            >
              {transcript || interim ? (
                ...
                <div className="h-[300px] flex flex-col items-center justify-center text-center text-text3 italic">
                  ...
                  <span>Tap "Speak Step {currentStepObj.stepNum}" and state your thoughts freely.</span>
                </div>
              )}
            </div>
          </div>
