...
export default function LadderSetup() {
  ...
  return (
    <div className="animate-fade-up w-full max-w-[760px] mx-auto px-6 pt-[60px] pb-20 max-[680px]:px-5">
      ...
      <div className="grid grid-cols-2 gap-3 mb-8 max-[680px]:grid-cols-1">
        {DOMAIN_PRESETS.map((preset) => (
          <div
            key={preset.id}
            onClick={() => handleSelectDomain(preset.label)}
            className={`bg-surface border rounded-xl p-5 cursor-pointer transition-all duration-[180ms] flex flex-col justify-between ${
              selectedDomain === preset.label
                ? 'border-accent-border bg-accent-dim shadow-lg'
                : 'border-border-md hover:border-border-hi hover:bg-surface2'
            }`}
          >
            <div className="flex items-center gap-3 mb-2">
              <span className="text-[24px]">{preset.icon}</span>
              <span className={`text-[15px] font-medium ${selectedDomain === preset.label ? 'text-accent' : 'text-text'}`}>
                {preset.label}
              </span>
            </div>
            <p className="text-[12px] text-text3 leading-[1.5]">{preset.hint}</p>
          </div>
        ))}
      </div>
      ...
      <div className="mb-9">
        ...
        <input
          ...
          className="w-full bg-surface border border-border-md rounded-xl p-[14px_18px] font-sans text-[14px] text-text outline-none transition-[border-color] duration-200 placeholder:text-text3 focus:border-accent-border"
        />
      </div>

      {/* Progression Preview Banner */}
      <div className="mb-9 p-6 bg-surface border border-border-md rounded-2xl">
        <div className="text-[10px] tracking-[0.18em] uppercase text-accent mb-3 font-semibold">
          How Difficulty Escalates (Endless Flow)
        </div>
        <div className="grid grid-cols-4 gap-2 max-[680px]:grid-cols-2">
          <div className="p-3 bg-surface2/60 border border-border rounded-xl">
            <span className="text-[10px] text-green-400 font-semibold block">LEVEL 1</span>
            <span className="text-[12px] text-text font-medium">Warm-Up</span>
          </div>
          <div className="p-3 bg-surface2/60 border border-border rounded-xl">
            <span className="text-[10px] text-amber-300 font-semibold block">LEVEL 2</span>
            <span className="text-[12px] text-text font-medium">Deep Analysis</span>
          </div>
          <div className="p-3 bg-surface2/60 border border-border rounded-xl">
            <span className="text-[10px] text-red-400 font-semibold block">LEVEL 3</span>
            <span className="text-[12px] text-text font-medium">Debate</span>
          </div>
          <div className="p-3 bg-surface2/60 border border-border rounded-xl">
            <span className="text-[10px] text-purple-300 font-semibold block">LEVEL 4+</span>
            <span className="text-[12px] text-text font-medium">Mastermind ∞</span>
          </div>
        </div>
      </div>
      ...
      <div ref={actionSectionRef} className="flex items-center gap-3 pt-2">
        <button
          ...
          className={`inline-flex items-center gap-2 bg-accent text-[#0e0e0d] border-none rounded-[10px] px-8 py-3.5 font-sans text-[14px] font-medium cursor-pointer transition-all duration-[180ms] tracking-[0.01em] hover:opacity-90 active:scale-[0.96] shadow-lg ${
